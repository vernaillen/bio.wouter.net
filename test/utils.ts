...
import { createPage, getBrowser, url, useTestContext } from '@nuxt/test-utils-edge'

export async function renderPage (path = '/') {
  const ctx = useTestContext()
  if (!ctx.options.browser) { throw new Error('`renderPage` require `options.browser` to be set') }
...
export async function withLogs (callback: (page: Page, logs: string[]) => Promise<void>) {
  ...
  page.on('console', (msg) => {
    const text = msg.text()
    if (done) { throw new Error('Test finished prematurely') }
